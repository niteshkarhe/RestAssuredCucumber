...
public class RequestBuilder extends Request
{
		...
		try
		{
			if(getHeaders().get(0)!=null)
			{
				response = given()
						...
						.accept(getContentType())
						.and()
					    .body(getRequestData().get("Request Body"))
					    .when()
					    .post(new URI(getURL()));
			}
			else if(getParams().get(0)!=null)
			{
	...
	public static void assertRespBodyTagValue(String tag, String value)
	{
		response
		.then()
		.body(tag, equalToIgnoringCase(getRequestData().get(value)));
	}
	...
	public static void verifyValueAtJsonPath(String value, String jsonpath)
	{
		...
		if(jsonPathEvaluator.get(jsonpath) instanceof ArrayList)
		{
			...
			Assert.assertTrue(list.contains(getRequestData().get(value)), "["+getRequestData().get(value)+"] is not present at jsonpath ["+jsonpath+"]");
		}
		else if(jsonPathEvaluator.get(jsonpath) instanceof String)
		{
			String actual = jsonPathEvaluator.get(jsonpath).toString();
			Assert.assertTrue(actual.contains(getRequestData().get(value)), "["+getRequestData().get(value)+"] is not present at jsonpath ["+jsonpath+"]");
		}
	}
	...
	public static void verifyValueAtXpathPath(String value, String xmlpath)
	{
		...
		if(jsonPathEvaluator.get(xmlpath) instanceof ArrayList)
		{
			...
			Assert.assertTrue(list.contains(getRequestData().get(value)), "["+getRequestData().get(value)+"] is not present at jsonpath ["+xmlpath+"]");
		}
		else if(jsonPathEvaluator.get(xmlpath) instanceof String)
		{
			String actual = jsonPathEvaluator.get(xmlpath).toString();
			System.out.println(actual);
			Assert.assertTrue(actual.contains(getRequestData().get(value)), "["+getRequestData().get(value)+"] is not present at jsonpath ["+xmlpath+"]");
		}
	}
